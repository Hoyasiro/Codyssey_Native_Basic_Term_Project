...
# 프로젝트 리드 전략
- 첫 term project로 아직 코디세이 교육 초반인 점을 감안하여 진지한 교육생의 옥석이 가려지지 않았다고 판단되어 참여 교육생의 관심도와 기여도를 사전 설문을 통해서 평가하였고, 개인 당 미션 수행에 소요되는 시간을 최소화(6시간 이내/1주)하고자 하였다. 적은 기여 시간 대비하여 고퀄리티의 작업물을 얻기 위해선 모든 팀원이 가용한 시간에 작업을 조각조각 수행할 수 있어야 한다고 판단하였고, AI 추천 작업 방식으로 병렬식 작업 수행을 추천하여 이를 도입하기로 결정하였다.
