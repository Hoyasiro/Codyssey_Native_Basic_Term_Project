...
# 담당자 선정
- 김주원님-Figma 프로토타입 구축: 사전 설문 조사에서 강점으로 'Figma 사용 이력'인 점을 고려하여 배정하였다.
- 육민호님-레이아웃 페이지 생성: 사전 설문 조사에서 강점으로 '이미지 생성 및 수정'을 기재하여 배정하였다.
- 김동호 팀장-로그문서 작성 및 html/css 구축: 바이브 코딩으로 html/css 빌드 경험을 바탕으로 배정하였다. 로그문서 작성은 프로젝트 전반적인 리딩 및 검토 과정에서 축척되는 작업 정보 등을 정리하여 최종 발표자료를 만들기 용이하여 배정하였다.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 [Phase 0] 공동 기획  ─ 김동호 리드 / 전원 참여
   · 사전 설문 → 역할 배정      · 서비스 컨셉 선정("러닝 크루 매칭 앱")
   · 화면 3역할 정의(메인/목록/상세)
   · 공통 스타일 기준 합의(민트 #2EC4B6, 하단 네비 4탭, 9:16)
       └ 이후 전 트랙이 공유하는 단일 기준점
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
                              │
        ┌─────────────────────┼─────────────────────┐
        ▼                     ▼                     ▼
┌───────────────────┐ ┌───────────────────┐ ┌───────────────────┐
│ Track A ─ 육민호   │ │ Track B ─ 김주원   │ │ Track C ─ 김동호   │
│ 이미지 생성        │ │ 프로토타입 설계    │ │ 문서 · 빌드        │
├───────────────────┤ ├───────────────────┤ ├───────────────────┤
│ 모델 탐색          │ │ 화면 전환 시나리오 │ │ 작업 로그 골격     │
│ SD1.5 → LoRA →    │ │ 설계               │ │ 작성               │
│ Ideogram → Bing/   │ │ (메인→목록→상세)   │ │                    │
│ Leonardo →         │ │                    │ │ HTML/CSS 스켈레톤  │
│ GPT Image 2(채택)  │ │ Figma 프레임·      │ │ 사전 구성          │
│                    │ │ 컴포넌트 사전 세팅 │ │ (레이아웃/네비)    │
│ 프롬프트 최적화    │ │                    │ │                    │
│ v1(초안) → v2      │ │ Hotspot 규격 정의  │ │ 제출물 체크리스트  │
│ 사유·결과 차이 기록│ │ (투명 영역 좌표)   │ │ 관리               │
│                    │ │                    │ │                    │
│ 일관성 확보        │ │ 후가공 방식 비교   │ │                    │
│ 시드 미지원 대응 → │ │ 검토(덮어쓰기 /    │ │                    │
│ 스타일 고정 문장 + │ │ 인페인팅 / 업스케일│ │                    │
│ 원본 PNG 보존      │ │ / 재생성)          │ │                    │
└─────────┬─────────┘ └─────────┬─────────┘ └─────────┬─────────┘
          │ v2 3장 산출          │ (대기 없이 선행 작업) │
          ▼                      │                       │
◆ 동기화 1 ─ 결과물 검토(김동호) ─────────────────────────┘
   main · list → 최종 확정 ┃ detail → 보완 지시
          │                              │
          │ (확정분 즉시 Track B 전달)   │ (보완 요청 → Track A)
          ▼                              ▼
   [Track B 선착수]              [Track A] 이미지 수정
   main/list Figma 배치           detail v2 → v3
   Hotspot 연결 시작              (프롬프트 기록)
          │                              │
          └──────────────┬───────────────┘
                         ▼
◆ 동기화 2 ─ 에셋 확정(3장 최종본)
        ...
        ┌────────────────┴────────────────┐
        ▼                                 ▼
 [Track B ─ 김주원]                 [Track C ─ 김동호]
  detail 배치 + 전체 Hotspot 완결     확정 에셋 기반
  후가공(총원 21명 통일,              HTML/CSS 빌드업
  개설일 2023.03 → 2026.03)          (RuncrewApp.html)
        │                                 │
        └────────────────┬────────────────┘
                         ▼
◆ 동기화 3 ─ 최종 통합 · 제출
   Figma 공유 링크 + 작업 로그(README.md) + RuncrewApp.html
```
---
## 1. 레이아웃 생성(담당자: 육민호)
