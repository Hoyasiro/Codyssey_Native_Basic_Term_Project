...
| 방식 | 채택 | 사유 |
|---|---|---|
| **Figma 텍스트 레이어 덮어쓰기** | **채택** | 덮어쓰면 문구가 100% 정확해지고, 톤 통일에도 기여,<br> Figma 작업량이 이미지 생성 대비 불균형한 문제 해결 |
| 인페인팅 (부분 재생성) | 부분 채택<br> v1→v2 | 텍스트가 아닌 그래픽 결함(아이콘 뭉개짐, 아바타 왜곡)에만 적용 가능 |
| 업스케일링 | 부분 채택<br> Detail v2→v3 | 원본 해상도가 충분해 해상도 부족이 결함 원인이 아니었음 |
| 프롬프트 재생성 | 미채택 | **시드 고정이 불가능**하므로(§2.1) 재생성 시 이미 확보한 3장의 톤 일관성이 붕괴됨 |

> 재생성이 아니라 덮어쓰기를 택한 이유가 곧 이 프로젝트의 일관성 유지 전략이다.<br>
> 후가공은 Figma 업무로 배정되어 다음 섹션에서 기술 예정이다.
