...
## 2-2. 레이아웃의 구성
   (0) 주제: 러닝 크루 모집을 위한 전용 어플리케이션, 적정 페이스와 러닝 시간/장소를 선택하여 그룹러닝 매칭을 중계한다.
   
   (1) Main 페이지: 상단 검색바, 중단 추천 그룹(소속 크루 아이콘, 총원) 표시, 하단 네비게이션바

   (2) List 페이지: 최상단 지역 검색 드롭다운 버튼, 그 아래 요일 선택 버튼 나열, 그 아래 검색바, 본문 노출 그룹 카드 나열, 하단 네비게이션바

   (3) Detail 페이지: 그룹 상세 페이지로 그룹 대표 사진 + 그룹명 + 소개글 노출, 하단 '참여 신청' 버튼, 최하단 네비게이션바
